...
class item_detail:
	...
	def process_data(self):
		...
		for i in range(len(item_sku)):
			if item_sku[i] in sku_detail_dict:
				...
				current_list = [customer_name[i],customer_email[i],purchase_qty[i]]
				...
				for temp_item_list in sku_temp_list:
					...
					if customer_email[i] == compare_email:
						...
						sku_detail_dict[item_sku[i]] = [customer_name[i], customer_email[i], qty]
						name_detail_dict[item_name[i]] = [customer_name[i], customer_email[i], qty]
						found = True
				...
				# current_list = 
				sku_detail_dict[item_sku[i]].append([customer_name[i], customer_email[i],purchase_qty[i]])
				name_detail_dict[item_name[i]] = []
				name_detail_dict[item_name[i]].append([customer_name[i], customer_email[i], purchase_qty[i]])
	...
	def main_displaygrid(self):
		...
		units_label = tk.Label(label_frame, text="Number of Units")
	...
	def showall(self):
		...
		self.displayEntries(self.name_list,self.email_list, self.qty_list)
	...
	def showdetails(self):
		...
		for i in range(len(self.name_list)):
			temp_list = [self.name_list[i], self.email_list[i], self.qty_list[i]]
	...
	def displayEntries(self,name_list,email_list, qty_list):
		...
		for i in range(len(email_list)):
			...
			email_entry = tk.Entry(self.entry_frame,font="Halvetica 10 bold", bd=2, width=45)
			...
			qty_entry = tk.Entry(self.entry_frame,font="Halvetica 10 bold", bd=2, width=12)
	...
	def exportDataToFile2(self):
		...
		fieldnames =["Customer Name","Email","Num of units"]
		if 1==1:
			with open(filename+'.csv','w', newline='') as file:
				...
				if type(self.hold_details[0])==list:
					for i in range(len(self.hold_details)):
						string_row = [str(self.hold_details[i][0]),str(self.hold_details[i][1]),str(self.hold_details[i][2])]
						writer.writerow(string_row)
				else:
					for i in range(1):
						string_row = [str(self.hold_details[0]),str(self.hold_details[1]),str(self.hold_details[2])]
						writer.writerow(string_row)
